...
async function autoCompleteCooking() {
    ...
    for (const orderId of orderIds) {
        try {
            await axios_1.default.post(`${kitchenUrl}/complete`, { order_id: orderId });
            success++;
        }
        catch (err) {
            console.error(`[workflow:auto-complete-cooking] order_id=${orderId} 실패 -`, err.message);
            failure++;
        }
    }
    console.log(`[workflow:auto-complete-cooking] 완료 - 성공:${success} 실패:${failure}`);
